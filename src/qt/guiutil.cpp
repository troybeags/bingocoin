...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bingocoin"))
        return false;
...
{
    // Convert bingocoin:// to bingocoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bingocoin://"))
    {
        uri.replace(0, 12, "bingocoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BingoCoin.lnk";
}
...
{
    return GetAutostartDir() / "bingocoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BingoCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bingocoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BingoCoin-Qt"));
